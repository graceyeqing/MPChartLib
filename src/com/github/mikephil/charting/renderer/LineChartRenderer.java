...
public class LineChartRenderer extends LineScatterCandleRadarRenderer {
    ...
    protected void drawCubicFill(LineDataSet dataSet, Path spline, Transformer trans,
        ...
        if (Build.VERSION.SDK_INT >= 18) {
            try {
                BarLineChartBase cc = (BarLineChartBase) mChart;
                Drawable drawable = ContextCompat.getDrawable(cc.getContext(), dataSet.getmDrawable());
                drawFilledPath(mBitmapCanvas, spline, drawable);
            }catch (Throwable e){
            ...
            for (int i = 0; i < dataSets.size(); i++) {
                ...
                for (int j = 0; j < positions.length; j += 2) {
                    ...
                    if(j == positions.length-2){
                        mValuePaint.setARGB(255,255,23,80);
                    }else{
                        mValuePaint.setARGB(255,204,204,204);
                    }
