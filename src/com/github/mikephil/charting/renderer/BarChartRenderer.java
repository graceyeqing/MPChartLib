...
public class BarChartRenderer extends DataRenderer {
    ...
    public static boolean isShowDarwText;
    ...
    @Override
    public void drawValues(Canvas c, Context context) {
        // if values are drawn
        if (passesCheck()) {
            ...
            for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < valuePoints.length * mAnimator.getPhaseX(); j += 2) {
                        ...
                        drawValue(context, c, formatter.getFormattedValue(val), valuePoints[j],
                    ...
                    for (int j = 0; j < (valuePoints.length - 1) * mAnimator.getPhaseX(); j += 2) {
                        ...
                        if (vals == null) {
                            ...
                            drawValue(context, c, formatter.getFormattedValue(e.getVal()), valuePoints[j],
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                drawValue(context, c, formatter.getFormattedValue(vals[k / 2]), x, y);
                            }
                        }
                    }
                }
     ...
     * @formatter
     */
    protected void drawValue(Context context, Canvas c, String value, float xPos, float yPos) {
        if(isShowDarwText){
            try {
                View tv = View.inflate(context,  R.layout.barchart_item, null);
                TextView textview = (TextView) tv.findViewById(R.id.chart_text);
                ...
                c.drawBitmap(bitmap, xPos - (bitmap.getWidth() / 2), yPos - bitmap.getHeight(), mValuePaint);
            }catch (Throwable e){
